...
import { FieldWrapProps, RangeFieldWrapProps, SelectFieldOptions, SelectFieldWrapProps } from '../../types'
...
const RangeInputSuffix = styled.span`
 ...
 align-items: center;
 height: 63px;
 background-color: ${(props: StyledProps): string => props.theme.submitButtonBgColor};
 border-radius: 5px;
 padding: 0 15px;
 font-weight: bold;
 font-size: 14px;
...
const SliderDiv = styled.div`
 .rangeslider {
     ...
     height: 12px;
     border-radius: 10px;
     background: ${(props: StyledProps): string => props.theme.inputPlaceHolderColor} 0% 0% no-repeat padding-box;
     .rangeslider__fill {
          height: 100%;
          background-color: ${(props: StyledProps): string => props.theme.submitButtonBgColor};
          border-radius: 10px;
     ...
     .rangeslider__handle {
          ...
          width: 24px;
          height: 24px;
          background: ${(props: StyledProps): string => props.theme.inputBgColor} 0% 0% no-repeat padding-box;
          position: absolute;
          cursor: pointer;
          ...
          &::after {
               width: 7px;
               height: 7px;
               background: ${(props: StyledProps): string => props.theme.submitButtonBgColor} 0% 0% no-repeat padding-box;
               position: absolute;
               content: '';
               top: 0;
               bottom: 0;
...
export const BaseRangeField: (props: RangeFieldWrapProps) => ReactElement = ({
    ...
    const handleOnChange = (e: ChangeEvent<HTMLInputElement>): void => {
        ...
        clearTimeout(timer)
        setValue(value)
        setTimer(setTimeout((value: number) => {
    ...
    return (
        <div>
            ...
            <StyledRow style={{ justifyContent: "space-between" }}>
                <StyledSpan>Min. {props.min} {props.unit}</StyledSpan>
                <StyledSpan>Max. {props.max} {props.unit}</StyledSpan>
            </StyledRow>
        </div>
    )
}
